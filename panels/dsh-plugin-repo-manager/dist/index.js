 ...
 */
export function skillsDirCandidates(explicit) {
    const raw = [];
    const push = (dir, source) => {
        if (dir && dir.trim())
            raw.push({ dir: expandHome(dir.trim()), source });
    };
...
export function resolveSkillsDirWithSource(ctx) {
    const config = ctx.get?.('config');
    const explicit = config?.['skillsDir'] ?? process.env['DSH_PLUGIN_SKILLS_DIR'];
    // ① 显式配置优先
    ...
    if (!process.env['DSH_HOME']) {
        ...
        if (withSkills.length === 1)
            return { dir: withSkills[0].dir, source: `${withSkills[0].source}(兜底)` };
    }
    return { dir: primary.dir, source: primary.source };
}
/**
 ...
 * 这种两处判据漂移。
 */
export function describeSkillsDir(skillsDir, source) {
    const candidates = skillsDirCandidates();
 ...
 */
export function resolveShowSidebarEntry(ctx) {
    const config = ctx.get?.('config');
    const fromConfig = config?.['showSidebarEntry'] !== undefined
        ? config['showSidebarEntry']
        : config?.['showSidebarButton'];
    const raw = fromConfig !== undefined
        ? fromConfig
...
export async function apply(ctx) {
    ...
    if (webServer) {
        webServer.register({
            ...
            handler: (req, res) => handleApiRequest(req, res, { repoDir, skillsDir, pollInterval, showSidebarEntry, sidebarTitle, skillsDirSource }),
        });
