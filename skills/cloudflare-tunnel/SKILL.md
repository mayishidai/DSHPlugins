...
All `scripts/...` and `references/...` paths in this document are relative to this skill's install directory, not the user's project. When the working directory is the user's project, call the helper with its full installed path.

Set `SKILL_DIR` once, picking the line that matches this install:

```bash
# SKILL_DIR="$(cygpath -u "$USERPROFILE")/.workbuddy/skills/cloudflare-tunnel"  # WorkBuddy (Windows Git Bash)
SKILL_DIR="$HOME/.workbuddy/skills/cloudflare-tunnel"                           # WorkBuddy (macOS/Linux)
# SKILL_DIR="$DSH_HOME/skills/cloudflare-tunnel"                               # DeepSeek Harness

python3 "$SKILL_DIR/scripts/tunnel_helper.py" quick --url http://localhost:3000
```

Every `python3 scripts/tunnel_helper.py ...` shown later in this document means the same helper;
prefix it with `"$SKILL_DIR/"` when your shell is not already inside the skill directory.
