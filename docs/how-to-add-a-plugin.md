...
- [ ] **动过凭据判据的话**：`make verify` 全绿（两套实现判定必须一致，
      否则其中一套失去信号价值）
- [ ] **动过宿主路径解析的话**：`make verify` 第 8 步全绿。该步会在**内存里**把判据
      改窄回「回退值只认 `/` 开头、不扫 `.py`」的失明状态，确认除形态①外的注入
      **都抓不到** —— 以此证明前面的「抓到了」不是凑巧。改动判据后这一步必须仍能
      区分开，否则说明守卫空转（2026-09-23：判据正是太窄，漏掉 `$HOME` / `.py`
      两种形态；修法与唯一实现见 `mcps/hindsight/scripts/hindsight_paths.py`）。
