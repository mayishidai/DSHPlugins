...
把 `scripts/lib.sh` 里的 `SKILLS_DIR` 改成你的目标目录即可；或用 `customSkillDirs` 配置指向仓库的 `plugins/` 目录直接扫描。
...
在 DSH 的 skill-filesystem 配置里，把本仓库的 `plugins/` 加进 `customSkillDirs`。这样 `plugins/` 下的技能会被实时发现，无需复制。安装脚本则用于「分发/打包」场景。